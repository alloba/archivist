...
export default class S3Destination {
    ...
    public async saveMedia(filemeta: FileMeta, rawfilepromise: Promise<Buffer>): Promise<void> {
        ...
        await this.s3.putObject({Bucket: this.bucket, Key: this.basepath + filename, Body: filecontents, ACL: 'public-read'}).promise()
        return Promise.resolve()
    }
    ...
    public async getExistingMeta(): Promise<FileMeta[]>{
        ...
        while(true){
            const datachunk: S3.ListObjectsV2Output = await this.s3.listObjectsV2({Delimiter: "", Bucket: this.bucket, Prefix: this.basepath, ContinuationToken: continuationToken}).promise();
